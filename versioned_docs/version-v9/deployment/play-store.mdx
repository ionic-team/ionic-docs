...
Select the Create new release button (it doesn't matter which release type, since you don't have to actually go through with creating a new release right now):
...
Follow the instructions on the screen to generate the Keystore and you can use the same Keystore file to sign your app in the Appflow dashboard as well. If you need any help generating the Keystore file, refer to our [Android certificates documentation](https://ionic.io/docs/appflow/package/credentials#android-certificates).
...
To opt into app signing, you'll need to upload the app signing key used to sign previous releases of the app. Navigate to Setup -> App integrity, then choose one of the two opt-in methods, **Export and upload a key from Android Studio** or **Export and upload a key from Java keystore**. Once the key has been uploaded, you can enroll in Play App Signing.
