...
Almost all tooling for modern JavaScript projects is based in [Node.js](../reference/glossary#node). The [download page](https://nodejs.org/en/download/) has prebuilt installation packages for all platforms. We recommend selecting the LTS version to ensure best compatibility.

Node is bundled with [npm](../reference/glossary#npm), the package manager for JavaScript.
...
Although not required, the version control system [Git](../reference/glossary#git) is highly recommended.
