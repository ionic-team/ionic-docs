...
1. Changing the default [Ionic colors](colors.md) for all [modes](platform-styles#ionic-modes) to complement the dark background in the `body.dark` selector.
...
:::note
For more information on the variables that are being changed, including other variables that can be added to further customize, see [Themes](themes).
:::
