...
## ディレクティブが動作しない {/* #directive-not-working */}
...
## Cordova プラグインがブラウザ上で動作しない {/* #cordova-plugins-not-working-in-the-browser */}
...
## provider の複数のインスタンス {/* #multiple-instances-of-a-provider */}
...
## 関数のコールバックで`this`にアクセスすると`undefined`が返されます {/* #accessing-this */}

Certain components, such as [counterFormatter on ion-input](../api/input#prop-counter-formatter) and [pinFormatter on ion-range](/api/range.mdx#prop-pin-formatter), allow developers to pass callbacks. It's important that you bind the correct `this` value if you plan to access `this` from within the context of the callback. You may need to access `this` when using Angular components or when using class components in React. There are two ways to bind `this`:
