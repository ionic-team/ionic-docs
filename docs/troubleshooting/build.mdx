...
## 一般的な間違い {/* #common-mistakes */}

### デコレーターにて丸括弧を忘れる {/* #forgetting-parentheses-on-a-decorator */}
...
### すべてのパラメータを解決できない {/* #cannot-resolve-all-parameters */}
...
パラメータが別のコンポーネントまたは Directive（たとえば、親コンポーネント）である場合、パラメータを providers のリストに追加するとエラーはなくなりますが、これは前述の [provider の複数のインスタンス](/troubleshooting/runtime.mdx#multiple-instances-of-a-provider) と同じ効果を持ちます。ここでは、コンポーネントクラスの新しいインスタンスを作成しますが、必要なコンポーネントインスタンスへの参照は取得しません。かわりに、注入されるであろう Directive またはコンポーネントがコンポーネントで使用可能であることを確認します（たとえば、親であることを期待している場合は、実際に親であること）。これはおそらく、例を使用すると最も簡単に理解できます:
...
### 既知のプロパティではないため、`propertyName` にバインドできない {/* #cant-bind-to-propertyname-since-it-isnt-a-known-property */}
...
### ControlContainer の Provider がない {/* #no-provider-for-controlcontainer */}
...
### コンポーネントファクトリがない {/* #no-component-factory-found */}
