...
Gallery Item is a child component of [Gallery](./gallery.mdx). It wraps the content for a single cell of the grid. The content can be plain text or any element, including an `img`, a `figure` with a caption, or a [Card](./card.mdx).
