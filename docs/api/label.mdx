...
Label is an element used primarily to add text content to [Item](./item) components. Label can also be used inside of form control components such as [Input](./input) or [Radio](./radio) when specifying the visible label, but it is not required.
