...
Rich content in select options is disabled by default. Set [`innerHTMLTemplatesEnabled`](/docs/developing/config.mdx#ionicconfig) to `true` in your [global Ionic config](/docs/developing/config.mdx#global-config). Markup inside options is treated as plain text when it is disabled. Refer to [Security](/docs/techniques/security.mdx) for sanitization guidance when enabling custom HTML.
