...
| <kbd>Ctrl</kbd> + <kbd>V</kbd> <br/> <kbd>Cmd</kbd> + <kbd>V</kbd> | Pastes content starting from the current box.<br/>If the pasted content is shorter than the remaining boxes, only available boxes are filled.<br/>If the pasted content is longer than the available boxes, the extra characters are ignored.<br/>If pasting a code when all boxes are filled, existing values up to the length of the pasted content are replaced, beginning with the first box, regardless of the focused box. |
