...
- **SSHクライアント**：[PuTTy](https://putty.software/)など、Appflowへの安全なログイン用。
- **Node.js**：Ionicエコシステムとやり取りするため。[LTSバージョンをダウンロード](https://nodejs.org/en/)。
- **コードエディタ**：コードを書くため! 私たちは[Visual Studio
  Code](https://code.visualstudio.com/)のファンです。
- **コマンドラインターミナル（CLI）**：ちなみに**Windows**
  ユーザーは、最高のIonic体験のために、管理者モードで実行する組み込みコマンドライン（cmd）またはPowershell
  CLIを推奨します。
  **Mac/Linux**ユーザーには、ほぼどのターミナルでも動作します。
