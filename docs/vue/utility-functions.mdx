...
:::note
`useBackButton` コールバックは、アプリが Capacitor または Cordova で実行されているときにのみ発火します。詳細については、[Capacitor および Cordova におけるハードウェアバックボタン](/developing/hardware-back-button.mdx#support)を参照してください。
:::
