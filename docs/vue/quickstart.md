...
2. Change all `.ts` files to `.js`. In a blank Ionic Vue app, this should only be `src/router/index.ts` and `src/main.ts`. If you're using tests, you also need to rename `tests/unit/example.spec.ts`.
...
5. Delete the `src/shims-vue.d.ts` and `tsconfig.json` files.
