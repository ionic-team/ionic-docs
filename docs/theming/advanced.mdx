...
メタタグ `theme-color` の `content` 値には、有効な [CSS Color](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value) を含める必要があり、CSS Variables を含めることはできません。
...
詳細については、[MDN のテーマカラードキュメント](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta/name/theme-color)を参照してください。
...
| `--ion-margin`              | [Margin 属性](../layout/css-utilities.mdx#margin)のマージンを調整                             |
| `--ion-padding`             | [Padding 属性](../layout/css-utilities.mdx#padding)のパディングを調整                         |
| `--ion-placeholder-opacity` | input、textarea、searchbar、select コンポーネントで使用されるプレースホルダーの不透明度を調整 |

### グリッド変数 {/* #grid-variables */}
...
## 既知の変数の制限 {/* #known-limitations-with-variables */}

### アルファ値の問題 {/* #the-alpha-problem */}

16 進数カラーのアルファ使用については、まだ完全な[ブラウザサポート](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value#Browser_compatibility)はありません。[`rgba()`](<https://developer.mozilla.org/en-US/docs/Web/CSS/color_value#rgb()_and_rgba()>) 関数は、`R, G, B, A`（赤、緑、青、アルファ）のフォーマットのみ利用可能です。次のコードは、`rgba()` に受け渡される正しい値と間違った値の例を示しています。
...
:::note
CSS 変数の取得方法や設定方法については、[CSS Variables](css-variables.mdx)のセクションを参照してください。
:::
...
### CSS カラー変数の変更 {/* #modifying-css-color-variables */}
...
これは通常は問題にはなりませんが、アプリケーションに動的なテーマカラーの設定が必要な場合は問題になります。Ionic では、これが[各色にバリエーションがある](colors.mdx#layered-colors)理由であり、テーマ設定に[stepped colors](themes.mdx#stepped-colors)が必要な理由でもあります。
