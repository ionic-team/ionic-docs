...
:::caution
ハイコントラストライトパレットは[core.css](../layout/global-stylesheets.mdx#corecss)の後にインポートする必要があり、
...
このアプローチは、`.ion-palette-high-contrast`クラスが`html`要素に設定されている場合にハイコントラストパレットをアクティブにします。このクラスは開発者が適用する必要があります。これは[`.ion-palette-dark`クラス](./dark-mode.mdx#css-class)と組み合わせて、条件付きでハイコントラストダークパレットを適用できます。
...
## Ionic ハイコントラストテーマのカスタマイズ {/* #customizing-ionic-high-contrast-theme */}
...
テーマ変数は、関連するハイコントラストパレットファイルをインポートすることで設定され、アプリにコピーする必要はありません。変更される変数、さらなるカスタマイズのための追加変数を含む詳細については、[Themes](themes.mdx)セクションを参照してください。
...
1. `:root`セレクタで、すべての[modes](platform-styles.mdx#ionic-modes)の[Ionic colors](colors.mdx)をハイコントラストパレットに補完するように設定します。[`:root`](https://developer.mozilla.org/en-US/docs/Web/CSS/:root)セレクタは、[specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity)が高いことを除いて、セレクタ`html`と同一です。
...
1. `.ion-palette-high-contrast`セレクタで、すべての[modes](platform-styles.mdx#ionic-modes)の[Ionic colors](colors.mdx)をハイコントラストパレットに補完するように設定します。`.ion-palette-high-contrast`クラスは、アプリの`html`要素に追加する必要があります。
