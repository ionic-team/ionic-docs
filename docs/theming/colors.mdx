...
各色は以下のプロパティで構成されています：`base`、`contrast`、`shade`、および`tint`。`base`および`contrast`の色には、同じ色を[rgb 形式](https://developer.mozilla.org/en-US/docs/Glossary/RGB)で表した`rgb`プロパティも必要です。`rgb`プロパティが必要な理由については、[アルファ問題](advanced.mdx#the-alpha-problem)を参照してください。以下のドロップダウンから、Ionic が提供するデフォルトカラーとそのバリエーションを確認できます。

<LayeredColorsSelect />

## 色の変更 {/* #modifying-colors */}
