...
## コミュニティメンテナンス {/* #community-maintenance */}
...
Ionic の後続バージョンは iOS 13 をサポートしていないことに注意してください。[モバイルサポート表](./browser-support.mdx#mobile-platforms)を参照してください。
...
- \*Cordova plugin support with Ionic Native and `@ionic/react` can be achieved using the [vanilla JavaScript implementation](/native.mdx)
