...
## Ionic でダイナミックフォントスケーリングを有効にする {/* #using-dynamic-font-scaling */}

### アプリケーションで有効にする {/* #enabling-in-an-application */}

ダイナミックフォントスケーリング は、[typography.css](/layout/global-stylesheets.mdx#typographycss) ファイルがインポートされている限り、デフォルトで有効になっています。このファイルをインポートすると `--ion-dynamic-font` 変数が定義され、ダイナミックフォントスケーリング が有効になります。推奨はされませんが、開発者はアプリケーションコード内でこの変数を `initial` に設定することで ダイナミックフォントスケーリング を無効にすることもできます。
...
### カスタムフォントファミリー {/* #custom-font-family */}
...
### `em`単位と`rem`単位 {/* #em-units-versus-rem-units */}
...
## Ionic でのダイナミックフォントスケーリングの仕組み {/* #how-dynamic-font-scaling-works-in-ionic */}
...
### 異なるプラットフォームでのモードの使用 {/* #using-modes-on-different-platforms */}
...
### ダイナミックフォントスケーリングが機能しない {/* #dynamic-font-scaling-is-not-working */}
...
2. [typography.css](/layout/global-stylesheets.mdx#typographycss) ファイルがインポートされているか確認してください。このファイルは動的フォントスケーリングを有効にするために必要です。
...
### Android で最大および最小フォントサイズが尊重されない {/* #maximum-and-minimum-font-sizes-are-not-being-respected-on-android */}
...
### ダイナミックフォントスケーリングが無効でもフォントサイズが大きい/小さい {/* #font-sizes-are-largersmaller-even-with-dynamic-font-scaling-disabled */}
...
### スケールされた Ionic iOS コンポーネントのフォントサイズがネイティブ iOS の同等物と完全に一致しない {/* #scaled-ionic-ios-component-font-sizes-do-not-exactly-match-native-ios-equivalents */}
...
### iOS で Ionic アプリのテキストサイズがダイナミックフォントスケーリングを有効にしたときに変更された {/* #the-text-size-in-my-ionic-app-on-ios-changed-when-enabling-dynamic-font-scaling */}
