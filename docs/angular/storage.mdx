...
## ローカルアプリケーション設定とデータ {/* #local-application-settings-and-data */}
...
## リレーショナルデータストレージ（モバイル専用） {/* #relational-data-storage-mobile-only */}
...
## 非リレーショナル型大容量データストレージ（モバイルおよび Web） {/* #non-relational-high-volume-data-storage-mobile-and-web */}
