...
**TEMPORARY link-resolution probe, do not merge.** Each link below points at the same generated page using a different form. Whichever ones navigate correctly are the forms that resolve in the Japanese build.

- A relative file path: [relative](../../native/camera.mdx)
- An absolute file path: [absolute](/docs/native/camera.mdx)
- A plain URL, no extension: [plain url](/docs/native/camera)
- A relative path with no extension: [relative no ext](../../native/camera)
