...
:::note
<<<<<<<< HEAD:versioned_docs/version-v8/updating/4-0.mdx
...
>>>>>>>> origin/main:docs/updating/4-0.mdx
:::

We suggest the following general process when migrating an existing application from Ionic 3 to 4:

1. Generate a new project using the `blank` starter (refer to [Starting an App](../developing/starting.mdx))
