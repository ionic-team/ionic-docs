...
#### Zoneless 変更検出 {/* #zoneless-change-detection */}
...
Zone.js がない場合、非同期コールバック（オーバーレイ結果の await、`setTimeout`、RxJS の購読、`Platform` イベント）からコンポーネントの状態を更新しても、Angular は自動では再描画しません。そのような場合は、signal または `ChangeDetectorRef.markForCheck()` で Angular に通知する必要があります。Ionic アプリで使うパターンについては、[Zoneless 変更検出](/angular/zoneless.mdx)を参照してください。
...
#### Angular 22 での OnPush 変更検出 {/* #onpush-change-detection-on-angular-22 */}
...
#### コンポーネントのインポート {/* #component-imports */}
...
#### IonicModule の非推奨化 {/* #ionicmodule-deprecation */}

`IonicModule` は Ionic 9 で非推奨となり、将来のメジャーバージョンで削除されます。引き続き完全に機能するため、直ちに対応する必要はありません。準備ができたら、スタンドアロンと NgModule ベースの両方のアプリで使える `provideIonicAngular()` へ移行してください。[Modules から Standalone への移行](/angular/build-options.mdx#migrating-from-modules-to-standalone)を参照してください。

#### CSS のインポート {/* #css-imports */}
...
#### 型付きオーバーレイフックの props {/* #typed-overlay-hook-props */}
...
#### ルート定義の変更 {/* #route-definition-changes */}
...
#### ネストしたルートのパス {/* #nested-route-paths */}
...
#### ルートパラメータへのアクセス {/* #accessing-route-parameters */}
...
#### RouteComponentProps の削除 {/* #routecomponentprops-removed */}
...
#### exact Prop の削除 {/* #exact-prop-removed */}
...
#### render Prop の削除 {/* #render-prop-removed */}
...
#### プログラムによるナビゲーション {/* #programmatic-navigation */}
...
#### カスタム history Prop の削除 {/* #custom-history-prop-removed */}
...
#### IonRedirect の削除 {/* #ionredirect-removed */}
...
#### パスの正規表現制約の削除 {/* #path-regex-constraints-removed */}
...
#### IonRoute API の変更 {/* #ionroute-api-changes */}
...
#### ナビゲーションガード内の `next()` に対する非推奨警告 {/* #deprecation-warning-for-next-in-navigation-guards */}
...
#### パッケージの exports {/* #package-exports */}
...
#### `handleBehavior` のデフォルト変更 {/* #handlebehavior-default-changed */}
...
#### ルーター統合の削除 {/* #router-integration-removed */}
...
#### `swipeBackEnabled` 設定の動作変更 {/* #swipebackenabled-config-behavior-change */}
...
#### スワイプで戻るの無効化 {/* #disabling-swipe-to-go-back */}
...
#### `ionChange` は値が変わったときのみ発火 {/* #ionchange-only-fires-when-the-value-changes */}
...
#### Action Sheet インターフェースの `selected` ロール削除 {/* #action-sheet-interface-selected-role-removed */}
...
## アップグレードのヘルプが必要ですか？ {/* #need-help-upgrading */}
