...
:::note
このガイドでは、アプリを最新バージョンの Ionic 6 に更新済みであることを前提としています。このガイドを開始する前に、[Ionic 6 へのアップグレード ガイド](./6-0) に従っていることを確認してください。
:::
...
1. `swipeToClose` プロパティの使用を削除します。 [canDismiss](https://ionicframework.com/docs/api/modal#preventing-a-modal-from-dismissing) プロパティを使用して、モーダルが解除できるタイミングを制御します。
2. `canDismiss` プロパティを `undefined` に設定するコードをすべて削除します。 `canDismiss`プロパティのデフォルトが`true`になったので、このコードはもう必要ありません。
